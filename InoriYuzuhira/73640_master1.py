...
ki = LINETCR.LINE()
ki.login(token="Emz3N6DuTSWU2ALykuB0.d+e26P5ydM1CTqiLx2ooea.ybZM8LG10LKIwTIC9WTGM7zLK2EfxsAUD7CtgwAnYM4=")
ki.loginResult()

kk = LINETCR.LINE()
kk.login(token="EmaFr2EGgT4DXq8mK2bc.cwogcSzbtJtLt+f0js39Ra.lhg3Vc3S8kQxOO8/wqFAJaK9Bpapf+gvNXq+hxGhJGU=")
kk.loginResult()

kc = LINETCR.LINE()
kc.login(token="EmKVxZ0OsVhpLHz7KpHa.ASXSVcykGHrpTqYjZb0l2G.yGFB+DDdAcywiiFobTzkLOV5UR0pclesitxnTjTTmbg=")
kc.loginResult()
...
wait = {
    ...
    'timeline':False,
    'autoAdd':True,
    'message':"Thankc for add",
    "lang":"JP",
    "comment":"Thankc for add me",
    "commentOn":True,
    "commentBlack":{},
    "wblack":False,
    "dblack":False,
...
def bot(op):
    try:
        ...
        if op.type == 25:
	       ...
	       if msg.from_ in admin:
              ...
              if msg.from_ in admin:
                ...
                msg.contentType = 13
                cl.sendText(msg.to, "Owner by Guntur_poenya_slera")
		...
		linkc = link_re.findall(msg.text)
		n_linkc=[]
		for l in linkc:
			if l not in n_linkc:
				n_linkc.append(l)
		for ticket_id in n_linkc:
			if wait["atjointicket"] == True:
				group=cl.findGroupByTicket(ticket_id)
				cl.acceptGroupInvitationByTicket(group.mid,ticket_id)
				cl.sendText(msg.to,"Sukces join ke grup %s" % str(group.name))
        ...
        #-------------Fungsi Jam on/off Start-------------------#            
         ...
         #-------------Fungsi Jam Update Start---------------------#            
                ...
                if wait["clock"] == True:
                    ...
                    profile.displayName = wait["cName4"] + nowT
                    kc.updateProfile(profile)
                    kc.sendText(msg.to,"Sukces update")
                else:
...
#-----------------------------------------------
                ...
                if msg.from_ in admin:
                   txt = msg.text.split(" ")
                   jmlh = int(txt[2])
                   tekc = msg.text.replace("Spam "+str(txt[1])+" "+str(jmlh)+ " ","")
                   tulisan = jmlh * (tekc+"\n")
                   #Keke cantik <3
                   if txt[1] == "on":
                        if jmlh <= 10000:
                             for x in range(jmlh):
                                   cl.sendText(msg.to, tekc)
                        else:
     ...
     for zx in range(0,20):
        ...
        if hasil['result']['posts'][zx]['postInfo']['liked'] == False:
          try:    
			...
			cl.comment(hasil['result']['posts'][zx]['userInfo']['mid'],hasil['result']['posts'][zx]['postInfo']['postId'],"Auto Like by k̶a̶n̶g̶ E̶d̶i̶t̶o̶r̶")
